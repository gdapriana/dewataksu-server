...
The Bookmark API provides endpoints for managing user likes on various content types including destinations, traditions, and stories. Users can bookmark and unbookmark content, with proper validation to prevent duplicate bookmarks.
...
Creates a new bookmark for a specific content item (destination, tradition, or story).
...
- `"destinations"` - For bookmarking destinations
- `"traditions"` - For bookmarking traditions
- `"stories"` - For bookmarking stories
...
- `200` - Content bookmarked successfully
...
- `id`: bookmark ID (string, cuid format)

**Example Request:**

```
DELETE /bookmark/clh7x2y3z0000like123
```
...
- `200` - Content unbookmarked successfully
- `400` - Invalid bookmark ID format
- `401` - Unauthorized (authentication required)
- `404` - Like not found
...
2. **Unlike the destination:**
   ```bash
   curl -X DELETE /bookmark/like_789ghi012jkl \
     -H "Authorization: Bearer your_jwt_token"
   ```
